...
import { useIncomeEntries, useScheduledIncomeEntries } from '@/hooks/use-year-data'
...
export default function IncomePage() {
  ...
  const oneTimeEntries = useIncomeEntries()
  const allScheduled   = useScheduledIncomeEntries()
  ...
  // Cancelled toggle
  const activeScheduled    = allScheduled.filter(e => !isCancelled(e))
  const cancelledScheduled = allScheduled.filter(e => isCancelled(e))
  const [showCancelled, setShowCancelled] = useState(false)

  // Recurring totals (active, non-once only)
  const recurActive = activeScheduled.filter(e => e.frequency !== 'once' && !e.end_date)
  ...
  const hasAnything = oneTimeEntries.length > 0 || allScheduled.length > 0
  ...
  const accent = KIND_META.scheduled_income.color
    ...
    <div className="min-h-full p-7">
      ...
      {cancelledScheduled.length > 0 && (
        <div className="mb-4">
          ...
          >
            ...
            {showCancelled ? 'Hide' : 'Show'} cancelled ({cancelledScheduled.length})
          </button>
        </div>
      )}
      {showCancelled && cancelledScheduled.length > 0 && (
        <RecurringSection
          title="Cancelled scheduled income"
          entries={cancelledScheduled}
          monthlyTotal={0}
          yearlyTotal={0}
          accentColor="#8a948c"
          accentSoft="#f4f3ef"
      ...
      {grouped.length > 0 && (
        <div className="mb-2 mt-7 flex items-center gap-2">
          <span className="text-[11px] font-bold uppercase tracking-[0.07em] text-bmuted">One-time income</span>
          <span className="rounded-full bg-bgreen/10 px-2 py-0.5 text-[11px] font-bold text-bgreen">{oneTimeEntries.length}</span>
        </div>
      )}
          ...
          <div key={month} className="mb-5">
            <div className="mb-2.5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="rounded-full bg-bgreen/10 px-2 py-0.5 text-[11px] font-bold text-bgreen">{items.length}</span>
                <Money value={monthTotal} auto className="text-[13px] font-bold text-bgreen" />
              </div>
            </div>
              ...
              {items.map((entry, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Money value={entry.amount} auto className="shrink-0 text-[13.5px] font-bold text-bgreen" />
      ...
      {!hasAnything && (
        <div className="rounded-[20px] border border-bline bg-bsurface px-6 py-14 text-center">
          <div className="text-[15px] font-semibold text-bink">No income yet</div>
          <div className="mt-1 text-[13px] text-bmuted">
            Add an income or scheduled income entry to start tracking your earnings.
          </div>
        </div>
      )}
