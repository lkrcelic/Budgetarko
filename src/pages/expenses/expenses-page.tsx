...
import { useExpenseEntries } from '@/hooks/use-year-data'
...
export default function ExpensesPage() {
  ...
  const grouped = useMemo(() => {
    ...
    // Sort entries within each month by amount descending
    ...
    <div className="min-h-full p-7">
          ...
          <div key={month} className="mb-5">
            ...
            <div className="mb-2.5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="rounded-full bg-bred/10 px-2 py-0.5 text-[11px] font-bold text-bred">
                  {items.length}
                </span>
                <Money
                  value={monthTotal}
                  auto
                  className="text-[13px] font-bold text-bred"
                />
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <Money
                      value={entry.amount}
                      auto
                      className="shrink-0 text-[13.5px] font-bold text-bred"
                    />
                    <button
                      type="button"
      ...
      {entries.length === 0 && (
        <div className="rounded-[20px] border border-bline bg-bsurface px-6 py-14 text-center">
          <div className="text-[15px] font-semibold text-bink">No expenses in {year}</div>
          <div className="mt-1 text-[13px] text-bmuted">
            Add an expense entry to start tracking your spending.
          </div>
        </div>
      )}
